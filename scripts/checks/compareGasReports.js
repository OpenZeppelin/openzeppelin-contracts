...
class Report {
    ...
    static compare(update, ref, opts = { hideEqual: true }) {
        ...
        // ... ... skip if the function doesn't exist in either contract, or if the report are identical and we have the hideEqual option
        // ... ... generate the log line
        // ... ... join the log lines
        // ... trim contracts for which there is no change
        const rows = Object.keys(update)
            ...
            .map(contract => unique(update[contract]
                ...
                .filter(([ current, previous ]) => current && previous && (!opts.hideEqual || JSON.stringify(current) !== JSON.stringify(previous)))
