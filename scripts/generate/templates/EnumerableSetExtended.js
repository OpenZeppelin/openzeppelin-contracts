...
const { EXTENDED_SET_TYPES } = require('./Enumerable.opts');
...
module.exports = format(
  ...
  format(
    [].concat(
      EXTENDED_SET_TYPES.filter(({ value }) => value.size == 0).map(set),
      EXTENDED_SET_TYPES.filter(({ value }) => value.size > 0).map(arraySet),
      hashes,
    ),
  ).trimEnd(),
  '}',
