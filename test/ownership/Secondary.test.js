...
contract.only('Secondary', function ([_, primary, newPrimary, other]) {
  ...
  describe('onlyPrimary', function () {
    ...
    it('reverts when anyone calls onlyPrimary functions', async function () {
      await shouldFail.reverting.withMessage(this.secondary.onlyPrimaryMock({ from: other }), "Secondary: caller is not the primary account");
    });
  });
  ...
  describe('transferPrimary', function () {
    ...
    it('reverts when transferring to the null address', async function () {
      await shouldFail.reverting.withMessage(this.secondary.transferPrimary(ZERO_ADDRESS, { from: primary }), "Secondary: new primary is the zero address");
    });

    it('reverts when called by anyone', async function () {
      await shouldFail.reverting.withMessage(this.secondary.transferPrimary(newPrimary, { from: other }), "Secondary: caller is not the primary account");
    });
    ...
    context('with new primary', function () {
      ...
      it('reverts when the old primary account calls onlyPrimary functions', async function () {
        await shouldFail.reverting.withMessage(this.secondary.onlyPrimaryMock({ from: primary }), "Secondary: caller is not the primary account");
      });
    });
  });
});
