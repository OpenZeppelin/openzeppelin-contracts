...
const clean = hex => (hex === '0x0' ? '0x' : ethers.toBeHex(hex));
...
const rlpEncodeBlock = block =>
  ethers.encodeRlp([
    ...
    block.receiptsRoot,
    block.logsBloom,
    clean(block.difficulty),
    clean(block.number),
    clean(block.gasLimit),
    clean(block.gasUsed),
    clean(block.timestamp),
    block.extraData,
    block.mixHash,
    block.nonce,
    clean(block.baseFeePerGas),
    block.withdrawalsRoot,
    clean(block.blobGasUsed),
    clean(block.excessBlobGas),
