...
contract('Create2', function (accounts) {
  ...
  describe('computeAddress', function () {
    it('computes the correct contract address', async function () {
      ...
      const offChainComputed = ethers.getCreate2Address(this.factory.address, saltHex, ethers.keccak256(constructorByteCode));
    ...
    it('computes the correct contract address with deployer', async function () {
      ...
      const offChainComputed = ethers.getCreate2Address(deployerAccount, saltHex, ethers.keccak256(constructorByteCode));
      expect(onChainComputed).to.equal(offChainComputed);
    });
  });
  ...
  describe('deploy', function () {
    it('deploys a contract without constructor', async function () {
      const offChainComputed = ethers.getCreate2Address(this.factory.address, saltHex, ethers.keccak256(ConstructorLessContract.bytecode));
