const { ethers } = require('hardhat');
const { expect } = require('chai');
const { loadFixture } = require('@nomicfoundation/hardhat-network-helpers');

const TEST_MESSAGE = ethers.id('OpenZeppelin');
const WRONG_MESSAGE = ethers.id('Nope');
const NON_HASH_MESSAGE = '0xabcd';

function toERC2098Format(signature) {
  return ethers.Signature.from(signature).compactSerialized;
}

function split(signature) {
  const raw = ethers.getBytes(signature);
  switch (raw.length) {
    case 64:
      return [
        ethers.hexlify(raw.slice(0, 32)), // r
        ethers.hexlify(raw.slice(32, 64)), // vs
      ];
    case 65:
      return [
        raw[64], // v
        ethers.hexlify(raw.slice(0, 32)), // r
        ethers.hexlify(raw.slice(32, 64)), // s
      ];
    default:
      expect.fail('Invalid signature length, cannot split');
  }
}

async function fixture() {
  const [other] = await ethers.getSigners();

  const ecdsa = await ethers.deployContract('$ECDSA');

  return { other, ecdsa };
}

describe('ECDSA', function () {
  beforeEach(async function () {
    Object.assign(this, await loadFixture(fixture));
  });

  describe('recover with invalid signature', function () {
    it('with short signature', async function () {
      await expect(this.ecdsa.$recover(TEST_MESSAGE, '0x1234'))
        .to.be.revertedWithCustomError(this.ecdsa, 'ECDSAInvalidSignatureLength')
        .withArgs(2);
    });

    it('with long signature', async function () {
      await expect(
        // eslint-disable-next-line max-len
        this.ecdsa.$recover(
          TEST_MESSAGE,
          '0x01234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789',
        ),
      )
        .to.be.revertedWithCustomError(this.ecdsa, 'ECDSAInvalidSignatureLength')
        .withArgs(85);
    });
  });

  describe('recover with valid signature', function () {
    describe('using <signer>.sign', function () {
      it('returns signer address with correct signature', async function () {
        // Create the signature
        const signature = await this.other.signMessage(TEST_MESSAGE);

        // Recover the signer address from the generated message and signature.
        expect(await this.ecdsa.$recover(ethers.hashMessage(TEST_MESSAGE), signature)).to.equal(this.other.address);
      });

      it('returns signer address with correct signature for arbitrary length message', async function () {
        // Create the signature
        const signature = await this.other.signMessage(NON_HASH_MESSAGE);

        // Recover the signer address from the generated message and signature.
        expect(await this.ecdsa.$recover(ethers.hashMessage(NON_HASH_MESSAGE), signature)).to.equal(this.other.address);
      });

      it('returns a different address', async function () {
        const signature = await this.other.signMessage(TEST_MESSAGE);
        expect(await this.ecdsa.$recover(WRONG_MESSAGE, signature)).to.not.equal(this.other.address);
      });

      it('reverts with invalid signature', async function () {
        // eslint-disable-next-line max-len
        const signature =
          '0x332ce75a821c982f9127538858900d87d3ec1f9f737338ad67cad133fa48feff48e6fa0c18abc62e42820f05943e47af3e9fbe306ce74d64094bdf1691ee53e01c';
        await expect(this.ecdsa.$recover(TEST_MESSAGE, signature)).to.be.revertedWithCustomError(
          this.ecdsa,
          'ECDSAInvalidSignature',
        );
      });
    });

    describe('with v=27 signature', function () {
      // Signature generated outside ganache with method web3.eth.sign(signer, message)
      const signer = '0x2cc1166f6212628A0deEf2B33BEFB2187D35b86c';
      // eslint-disable-next-line max-len
      const signatureWithoutV =
        '0x5d99b6f7f6d1f73d1a26497f2b1c89b24c0993913f86e9a2d02cd69887d9c94f3c880358579d811b21dd1b7fd9bb01c1d81d10e69f0384e675c32b39643be892';

      it('works with correct v value', async function () {
        const v = '1b'; // 27 = 1b.
        const signature = signatureWithoutV + v;
        expect(await this.ecdsa.$recover(TEST_MESSAGE, signature)).to.equal(signer);

        expect(
          await this.ecdsa.getFunction('$recover(bytes32,uint8,bytes32,bytes32)')(TEST_MESSAGE, ...split(signature)),
        ).to.equal(signer);

        expect(
          await this.ecdsa.getFunction('$recover(bytes32,bytes32,bytes32)')(
            TEST_MESSAGE,
            ...split(toERC2098Format(signature)),
          ),
        ).to.equal(signer);
      });

      it('rejects incorrect v value', async function () {
        const v = '1c'; // 28 = 1c.
        const signature = signatureWithoutV + v;
        expect(await this.ecdsa.$recover(TEST_MESSAGE, signature)).to.not.equal(signer);

        expect(
          await this.ecdsa.getFunction('$recover(bytes32,uint8,bytes32,bytes32)')(TEST_MESSAGE, ...split(signature)),
        ).to.not.equal(signer);

        expect(
          await this.ecdsa.getFunction('$recover(bytes32,bytes32,bytes32)')(
            TEST_MESSAGE,
            ...split(toERC2098Format(signature)),
          ),
        ).to.not.equal(signer);
      });

      it('reverts wrong v values', async function () {
        for (const v of ['00', '01']) {
          const signature = signatureWithoutV + v;
          await expect(this.ecdsa.$recover(TEST_MESSAGE, signature)).to.be.revertedWithCustomError(
            this.ecdsa,
            'ECDSAInvalidSignature',
          );

          await expect(
            this.ecdsa.getFunction('$recover(bytes32,uint8,bytes32,bytes32)')(TEST_MESSAGE, ...split(signature)),
          ).to.be.revertedWithCustomError(this.ecdsa, 'ECDSAInvalidSignature');
        }
      });

      it('rejects short EIP2098 format', async function () {
        const v = '1b'; // 27 = 1b.
        const signature = signatureWithoutV + v;
        await expect(this.ecdsa.$recover(TEST_MESSAGE, toERC2098Format(signature)))
          .to.be.revertedWithCustomError(this.ecdsa, 'ECDSAInvalidSignatureLength')
          .withArgs(64);
      });
    });

    describe('with v=28 signature', function () {
      const signer = '0x1E318623aB09Fe6de3C9b8672098464Aeda9100E';
      // eslint-disable-next-line max-len
      const signatureWithoutV =
        '0x331fe75a821c982f9127538858900d87d3ec1f9f737338ad67cad133fa48feff48e6fa0c18abc62e42820f05943e47af3e9fbe306ce74d64094bdf1691ee53e0';

      it('works with correct v value', async function () {
        const v = '1c'; // 28 = 1c.
        const signature = signatureWithoutV + v;
        expect(await this.ecdsa.$recover(TEST_MESSAGE, signature)).to.equal(signer);

        expect(
          await this.ecdsa.getFunction('$recover(bytes32,uint8,bytes32,bytes32)')(TEST_MESSAGE, ...split(signature)),
        ).to.equal(signer);

        expect(
          await this.ecdsa.getFunction('$recover(bytes32,bytes32,bytes32)')(
            TEST_MESSAGE,
            ...split(toERC2098Format(signature)),
          ),
        ).to.equal(signer);
      });

      it('rejects incorrect v value', async function () {
        const v = '1b'; // 27 = 1b.
        const signature = signatureWithoutV + v;
        expect(await this.ecdsa.$recover(TEST_MESSAGE, signature)).to.not.equal(signer);

        expect(
          await this.ecdsa.getFunction('$recover(bytes32,uint8,bytes32,bytes32)')(TEST_MESSAGE, ...split(signature)),
        ).to.not.equal(signer);

        expect(
          await this.ecdsa.getFunction('$recover(bytes32,bytes32,bytes32)')(
            TEST_MESSAGE,
            ...split(toERC2098Format(signature)),
          ),
        ).to.not.equal(signer);
      });

      it('reverts invalid v values', async function () {
        for (const v of ['00', '01']) {
          const signature = signatureWithoutV + v;
          await expect(this.ecdsa.$recover(TEST_MESSAGE, signature)).to.be.revertedWithCustomError(
            this.ecdsa,
            'ECDSAInvalidSignature',
          );

          await expect(
            this.ecdsa.getFunction('$recover(bytes32,uint8,bytes32,bytes32)')(TEST_MESSAGE, ...split(signature)),
          ).to.be.revertedWithCustomError(this.ecdsa, 'ECDSAInvalidSignature');
        }
      });

      it('rejects short EIP2098 format', async function () {
        const v = '1c'; // 27 = 1b.
        const signature = signatureWithoutV + v;
        await expect(this.ecdsa.$recover(TEST_MESSAGE, toERC2098Format(signature)))
          .to.be.revertedWithCustomError(this.ecdsa, 'ECDSAInvalidSignatureLength')
          .withArgs(64);
      });
    });

    it('reverts with high-s value signature', async function () {
      const message = '0xb94d27b9934d3e08a52e52d7da7dabfac484efe37a5380ee9088f7ace2efcde9';
      // eslint-disable-next-line max-len
      const highSSignature =
        '0xe742ff452d41413616a5bf43fe15dd88294e983d3d36206c2712f39083d638bde0a0fc89be718fbc1033e1d30d78be1c68081562ed2e97af876f286f3453231d1b';

      const [v, r, s] = split(highSSignature);
      await expect(this.ecdsa.$recover(message, highSSignature))
        .to.be.revertedWithCustomError(this.ecdsa, 'ECDSAInvalidSignatureS')
        .withArgs(s);
      await expect(this.ecdsa.getFunction('$recover(bytes32,uint8,bytes32,bytes32)')(TEST_MESSAGE, v, r, s))
        .to.be.revertedWithCustomError(this.ecdsa, 'ECDSAInvalidSignatureS')
        .withArgs(s);
      expect(() => toERC2098Format(highSSignature)).to.throw('non-canonical s');
    });
  });
});
