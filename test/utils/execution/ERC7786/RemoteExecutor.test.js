...
describe('RemoteExecutor', function () {
  ...
  it('reverts when calling contract with empty data (no fallback)', async function () {
    ...
    await expect(remoteExecutor.execute(await target.getAddress(), '0x')).to.be.revertedWith(
      'RemoteExecutor: call failed',
    );
  });

  it('reverts if non-owner calls execute', async function () {
    const data = target.interface.encodeFunctionData('setValue', [42]);
    await expect(remoteExecutor.connect(addr1).execute(await target.getAddress(), data))
  ...
  it('reverts if target is zero address', async function () {
    await expect(remoteExecutor.execute(ethers.ZeroAddress, '0x')).to.be.revertedWith(
      'RemoteExecutor: target is zero address',
    );
  });
  ...
  it('reverts if call fails', async function () {
    ...
    await expect(remoteExecutor.execute(await target.getAddress(), invalidData)).to.be.reverted; // Will bubble up the actual revert reason
  });
  ...
  it("bubbles the target's revert reason when available", async function () {
    ...
    await expect(remoteExecutor.execute(await target.getAddress(), data)).to.be.revertedWith(
  ...
  it('returns correct value from view function', async function () {
    ...
    const [success, result] = await remoteExecutor.execute.staticCall(await target.getAddress(), viewData);
  ...
  it('returns (true, bytes) and decodes correctly for view calls', async function () {
    ...
    const [success, result] = await remoteExecutor.execute.staticCall(await target.getAddress(), data);
  ...
  it('handles successful execution with return data', async function () {
    ...
    const [success] = await remoteExecutor.execute.staticCall(await target.getAddress(), data);
    expect(success).to.be.true;
  });
});
