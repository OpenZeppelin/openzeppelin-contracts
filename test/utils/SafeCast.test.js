...
contract("SafeCast", async (accounts) => {
  ...
  describe("castU64()", () => {
    ...
    it("passes where appropriate (i.e. 0 <= input < 2^64)", async () => {
      ...
      assert.equal(
        (await contract.castU64("18446744073709551615")).toString(), 
  ...
  describe("castU8()", () => {
    it("reverts on overflow (i.e. input >= 2^8)", async () => {
      const contract = await SafeCastMock.new()
      await truffleAssert.reverts(contract.castU8(256), "SafeCast: downcast overflow")
      await truffleAssert.reverts(contract.castU8(257), "SafeCast: downcast overflow")
    })
