      ...
      if (isValueType) {
        ...
        describe('splice with replacement', function () {
          ...
          describe('implicit start', function () {
            const fragment = `$splice(${name}[] arr, ${name}[] replacement)`;
            ...
            it('replacement longer than array', async function () {
              ...
              const replacement = Array.from({ length }, generators[name]);

              // create copy of array and splice it
              const expected = Array.from(array);
              expected.splice(start, replacement.length, ...replacement.slice(0, Math.max(expected.length - start, 0)));

              await expect(this.mock[fragment](array, replacement)).to.eventually.deep.equal(expected);
            });
            ...
            it('empty replacement', async function () {
              ...
              await expect(this.mock[fragment](array, replacement)).to.eventually.deep.equal(expected);
              expect(expected).to.deep.equal(array);
            });
            ...
            it('empty array', async function () {
              ...
              await expect(this.mock[fragment](emptyArray, replacement)).to.eventually.deep.equal(emptyArray);
            });
          });

          describe('explicit start', function () {
            const fragment = `$splice(${name}[] arr, uint256 start, ${name}[] replacement)`;
            ...
            it('replace at start', async function () {
              ...
              await expect(this.mock[fragment](array, start, replacement)).to.eventually.deep.equal(expected);
            });
            ...
            it('replace entire array', async function () {
              ...
              await expect(this.mock[fragment](array, start, replacement)).to.eventually.deep.equal(expected);
              expect(expected).to.deep.equal(replacement.slice(0, array.length));
            });
