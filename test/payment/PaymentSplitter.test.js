...
contract.only('PaymentSplitter', function ([_, owner, payee1, payee2, payee3, nonpayee1, payer1]) {
  ...
  it('rejects null payees', async function () {
    await shouldFail.reverting.withMessage(PaymentSplitter.new([payee1, ZERO_ADDRESS], [20, 30]), "PaymentSplitter: account is the zero address");
  });

  it('rejects zero-valued shares', async function () {
    await shouldFail.reverting.withMessage(PaymentSplitter.new([payee1, payee2], [20, 0]), "PaymentSplitter: shares are 0");
  });

  it('rejects repeated payees', async function () {
    await shouldFail.reverting.withMessage(PaymentSplitter.new([payee1, payee1], [20, 30]), "PaymentSplitter: account already has shares");
  });
  ...
  context('once deployed', function () {
    ...
    it('should throw if no funds to claim', async function () {
      await shouldFail.reverting.withMessage(this.contract.release(payee1), "PaymentSplitter: account is not due payment");
    });

    it('should throw if non-payee want to claim', async function () {
      await send.ether(payer1, this.contract.address, amount);
      await shouldFail.reverting.withMessage(this.contract.release(nonpayee1), "PaymentSplitter: account has no shares");
    });
