...
contract('PullPayment', function (accounts) {
  ...
  it('can record an async payment correctly', async function () {
    ...
    paymentsToAccount0.should.be.bignumber.eq(AMOUNT);
  });
  ...
  it('can add multiple balances on one account', async function () {
    ...
    const paymentsToAccount0 = await this.contract.payments(accounts[0]);
    paymentsToAccount0.should.be.bignumber.eq(500);
  });
  ...
  it('can add balances on multiple accounts', async function () {
    ...
    const paymentsToAccount0 = await this.contract.payments(accounts[0]);
    paymentsToAccount0.should.be.bignumber.eq(200);

    const paymentsToAccount1 = await this.contract.payments(accounts[1]);
    paymentsToAccount1.should.be.bignumber.eq(300);
  });
