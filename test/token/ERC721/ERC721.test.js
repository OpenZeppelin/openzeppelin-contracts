...
contract.only('ERC721', function ([_, creator, tokenOwner, other, ...accounts]) {
  ...
  describe('internal functions', function () {
    ...
    describe('_mint(address, uint256)', function () {
      it('reverts with a null destination address', async function () {
        await shouldFail.reverting.withMessage(this.token.mint(ZERO_ADDRESS, tokenId), "ERC721: mint to the zero address");
      });
    ...
    describe('_burn(address, uint256)', function () {
      it('reverts when burning a non-existent token id', async function () {
        await shouldFail.reverting.withMessage(this.token.methods['burn(address,uint256)'](tokenOwner, tokenId), "ERC721: owner query of nonexistent token");
      });
      ...
      context('with minted token', function () {
        ...
        it('reverts when the account is not the owner', async function () {
          await shouldFail.reverting.withMessage(this.token.methods['burn(address,uint256)'](other, tokenId), "ERC721: burn of unowned token");
        });
        ...
        context('with burnt token', function () {
          ...
          it('deletes the token', async function () {
            ...
            await shouldFail.reverting.withMessage(this.token.ownerOf(tokenId), "ERC721: owner query of nonexistent token");
          });
    ...
    describe('_burn(uint256)', function () {
      it('reverts when burning a non-existent token id', async function () {
        await shouldFail.reverting.withMessage(this.token.methods['burn(uint256)'](tokenId), "ERC721: owner query of nonexistent token");
      });
