...
describe('ERC1363', function () {
  ...
  describe('transferAndCall', function () {
    it('to an EOA', async function () {
      ...
      .to.be.revertedWithCustomError(this.token, 'ERC1363InvalidReceiver').withArgs(this.other.address);
    });

    it('without data', async function () {
      await expect(this.token.connect(this.holder).getFunction('transferAndCall(address,uint256)')(this.receiver, value))
      .to.emit(this.token, 'Transfer').withArgs(this.holder.address, this.receiver.target, value)
      .to.emit(this.receiver, 'Received').withArgs(this.holder.address, this.holder.address, value, '0x');
    });

    it('with data', async function () {
      await expect(this.token.connect(this.holder).getFunction('transferAndCall(address,uint256,bytes)')(this.receiver, value, data))
      .to.emit(this.token, 'Transfer').withArgs(this.holder.address, this.receiver.target, value)
      .to.emit(this.receiver, 'Received').withArgs(this.holder.address, this.holder.address, value, data);
    });
    ...
    it('with reverting hook (without reason)', async function () {
      ...
      await expect(this.token.connect(this.holder).getFunction('transferAndCall(address,uint256,bytes)')(this.receiver, value, data))
      .to.be.revertedWithCustomError(this.token, 'ERC1363InvalidReceiver').withArgs(this.receiver.target);
    });
    ...
    it('with reverting hook (with custom error)', async function () {
      ...
      await expect(this.token.connect(this.holder).getFunction('transferAndCall(address,uint256,bytes)')(this.receiver, value, data))
      .to.be.revertedWithCustomError(this.receiver, 'CustomError').withArgs(reason);
    });
  ...
  describe('transferFromAndCall', function () {
    ...
    it('to an EOA', async function () {
      await expect(this.token.connect(this.other).getFunction('transferFromAndCall(address,address,uint256)')(this.holder, this.other, value))
      .to.be.revertedWithCustomError(this.token, 'ERC1363InvalidReceiver').withArgs(this.other.address);
    });

    it('without data', async function () {
      await expect(this.token.connect(this.other).getFunction('transferFromAndCall(address,address,uint256)')(this.holder, this.receiver, value))
      .to.emit(this.token, 'Transfer').withArgs(this.holder.address, this.receiver.target, value)
      .to.emit(this.receiver, 'Received').withArgs(this.other.address, this.holder.address, value, '0x');
    });

    it('with data', async function () {
      await expect(this.token.connect(this.other).getFunction('transferFromAndCall(address,address,uint256,bytes)')(this.holder, this.receiver, value, data))
      .to.emit(this.token, 'Transfer').withArgs(this.holder.address, this.receiver.target, value)
      .to.emit(this.receiver, 'Received').withArgs(this.other.address, this.holder.address, value, data);
    });
  ...
  describe('approveAndCall', function () {
    it('an EOA', async function () {
      ...
      .to.be.revertedWithCustomError(this.token, 'ERC1363InvalidSpender').withArgs(this.other.address);
    });

    it('without data', async function () {
      await expect(this.token.connect(this.holder).getFunction('approveAndCall(address,uint256)')(this.spender, value))
      .to.emit(this.token, 'Approval').withArgs(this.holder.address, this.spender.target, value)
      .to.emit(this.spender, 'Approved').withArgs(this.holder.address, value, '0x');
    });

    it('with data', async function () {
      await expect(this.token.connect(this.holder).getFunction('approveAndCall(address,uint256,bytes)')(this.spender, value, data))
      .to.emit(this.token, 'Approval').withArgs(this.holder.address, this.spender.target, value)
      .to.emit(this.spender, 'Approved').withArgs(this.holder.address, value, data);
    });
    ...
    it('with reverting hook (without reason)', async function () {
      ...
      await expect(this.token.connect(this.holder).getFunction('approveAndCall(address,uint256,bytes)')(this.spender, value, data))
      .to.be.revertedWithCustomError(this.token, 'ERC1363InvalidSpender').withArgs(this.spender.target);
    });
    ...
    it('with reverting hook (with custom error)', async function () {
      ...
      await expect(this.token.connect(this.holder).getFunction('approveAndCall(address,uint256,bytes)')(this.spender, value, data))
      .to.be.revertedWithCustomError(this.spender, 'CustomError').withArgs(reason);
    });
