...
function shouldRevertOnAllCalls () {
  it('reverts on transfer', async function () {
    await shouldFail.reverting.withMessage(this.wrapper.transfer());
  });

  it('reverts on transferFrom', async function () {
    await shouldFail.reverting.withMessage(this.wrapper.transferFrom());
  });

  it('reverts on approve', async function () {
    await shouldFail.reverting.withMessage(this.wrapper.approve(0));
  });

  it('reverts on increaseAllowance', async function () {
    await shouldFail.reverting.withMessage(this.wrapper.increaseAllowance(0));
  });

  it('reverts on decreaseAllowance', async function () {
    await shouldFail.reverting.withMessage(this.wrapper.decreaseAllowance(0));
  });
}
  ...
  describe('approvals', function () {
    context('with zero allowance', function () {
      ...
      it('reverts when decreasing the allowance', async function () {
        await shouldFail.reverting.withMessage(this.wrapper.decreaseAllowance(10));
      });
    });
    ...
    context('with non-zero allowance', function () {
      ...
      it('reverts when approving a non-zero allowance', async function () {
        await shouldFail.reverting.withMessage(this.wrapper.approve(20));
      });
      ...
      it('reverts when decreasing the allowance to a negative value', async function () {
        await shouldFail.reverting.withMessage(this.wrapper.decreaseAllowance(200));
      });
    });
  });
}
