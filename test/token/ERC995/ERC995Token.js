...
var ERC995TokenMock = artifacts.require('ERC995TokenMock');
...
contract('ERC995 Token', function (accounts) {
  let token;
  ...
  beforeEach(async function () {
    token = await ERC995TokenMock.new(accounts[0], 100);
  });
  ...
  describe('Test ERC995 methods', function () {
    it(
      ...
      , async function () {
        ...
        const preData = message.contract.showMessage.getData(
          web3.toHex(123456), 666, 'Transfer to be done'
        );
        const posData = message.contract.showMessage.getData(
          web3.toHex(987654), 333, 'Transfer done'
        );
        const abiMethod = findMethod(token.abi, 'transfer', 'address,address,uint256,bytes,bytes');
        const transferData = ethjsABI.encodeMethod(abiMethod,
          [message.contract.address, message.contract.address, 100, preData, posData]
        );
    ...
    it(
      ...
      , async function () {
        ...
        const abiMethod = findMethod(token.abi, 'approve', 'address,address,uint256,bytes,bytes');
    ...
    it(
      ...
      , async function () {
        ...
        const abiMethod = findMethod(token.abi, 'increaseApproval', 'address,uint256,address,bytes,bytes');
        const increaseApprovalData = ethjsABI.encodeMethod(abiMethod,
          [message.contract.address, 50, message.contract.address, preData, posData]
        );
        const transaction = await token.sendTransaction(
          { from: accounts[0], data: increaseApprovalData }
        );
    ...
    it(
      ...
      , async function () {
        ...
        const abiMethod = findMethod(token.abi, 'decreaseApproval', 'address,uint256,address,bytes,bytes');
        const decreaseApprovalData = ethjsABI.encodeMethod(abiMethod,
          [message.contract.address, 60, message.contract.address, preData, posData]
        );
        const transaction = await token.sendTransaction(
    ...
    it(
      ...
      , async function () {
        ...
        const abiMethod = findMethod(token.abi, 'transferFrom', 'address,address,address,uint256,bytes,bytes');
        const transferFromData = ethjsABI.encodeMethod(abiMethod,
          [accounts[0], message.contract.address, message.contract.address, 100, preData, posData]
        );
        const transaction = await token.sendTransaction(
    ...
    it('should fail inside approve (with data)', async function () {
      ...
      const preData = message.contract.fail.getData();
      const posData = message.contract.fail.getData();

      const abiMethod = findMethod(token.abi, 'approve', 'address,address,uint256,bytes,bytes');
      const approveData = ethjsABI.encodeMethod(abiMethod,
        [message.contract.address, message.contract.address, 10, preData, posData]
      );
      await token.sendTransaction(
        { from: accounts[0], data: approveData }
      ).should.be.rejectedWith(EVMRevert);
    ...
    it('should fail inside transferFrom (with data)', async function () {
      ...
      const abiMethod = findMethod(token.abi, 'transferFrom', 'address,address,address,uint256,bytes,bytes');
      const transferFromData = ethjsABI.encodeMethod(abiMethod,
        [accounts[2], message.contract.address, message.contract.address, 10, preData, posData]
      );
      await token.sendTransaction(
        { from: accounts[1], data: transferFromData }
      ).should.be.rejectedWith(EVMRevert);
    ...
    it('should fail approve (with data) when using token contract address as receiver', async function () {
      ...
      const abiMethod = findMethod(token.abi, 'approve', 'address,address,uint256,bytes,bytes');
      const approveData = ethjsABI.encodeMethod(abiMethod,
        [token.contract.address, token.contract.address, 100, preData, posData]
      );
      await token.sendTransaction(
        { from: accounts[0], data: approveData }
      ).should.be.rejectedWith(EVMRevert);
    ...
    it('should fail transferFrom (with data) when using token contract address as receiver', async function () {
      ...
      const abiMethod = findMethod(token.abi, 'transferFrom', 'address,address,address,uint256,bytes,bytes');
      const transferFromData = ethjsABI.encodeMethod(abiMethod,
        [accounts[0], token.contract.address, token.contract.address, 1, preData, posData]
      );
      await token.sendTransaction(
        { from: accounts[1], data: transferFromData }
      ).should.be.rejectedWith(EVMRevert);
