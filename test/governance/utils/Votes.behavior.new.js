...
function shouldBehaveLikeVotes(tokens, { mode = 'blocknumber', fungible = true }) {
  beforeEach(async function () {
    ...
    this.alice = this.accounts[0];
    this.bob = this.accounts[1];
    this.types = {
      Delegation: [
  ...
  describe('run votes workflow', function () {
    it('initial nonce is 0', async function () {
      expect(await this.votes.nonces(this.alice)).to.equal(0);
    });
    ...
    describe('delegation with signature', function () {
      ...
      it('delegation without tokens', async function () {
        expect(await this.votes.delegates(this.alice)).to.be.equal(ethers.ZeroAddress);

        const delegate = await this.votes.connect(this.alice).delegate(this.alice);
        await expect(delegate)
          .to.emit(this.votes, 'DelegateChanged')
          .withArgs(this.alice.address, ethers.ZeroAddress, this.alice.address);
        await expect(delegate).not.to.emit(this.votes, 'DelegateVotesChanged');

        expect(await this.votes.delegates(this.alice)).to.be.equal(this.alice.address);
      });
      ...
      it('delegation with tokens', async function () {
        ...
        const delegate = await this.votes.connect(this.alice).delegate(this.alice);
        const timepoint = await clockFromReceipt[mode](delegate);

        await expect(delegate)
          .to.emit(this.votes, 'DelegateChanged')
          .withArgs(this.alice.address, ethers.ZeroAddress, this.alice.address);
        await expect(delegate).to.emit(this.votes, 'DelegateVotesChanged').withArgs(this.alice.address, 0, weight);

        expect(await this.votes.delegates(this.alice)).to.be.equal(this.alice.address);
        expect(await this.votes.getVotes(this.alice)).to.equal(weight);
        expect(await this.votes.getPastVotes(this.alice, timepoint - 1n)).to.equal(0);
        await mine();
      ...
      it('delegation update', async function () {
        ...
        await expect(delegate).to.emit(this.votes, 'DelegateVotesChanged').withArgs(this.alice.address, weight, 0);
        await expect(delegate).to.emit(this.votes, 'DelegateVotesChanged').withArgs(this.bob.address, 0, weight);

        expect(await this.votes.delegates(this.alice)).to.be.equal(this.bob.address);
        expect(await this.votes.getVotes(this.alice)).to.equal(0);
        ...
        expect(await this.votes.getPastVotes(this.bob, timepoint - 1n)).to.equal(0);
        await mine();
        expect(await this.votes.getPastVotes(this.alice, timepoint)).to.equal(0);
      ...
      describe('with signature', function () {
        ...
        it('accept signed delegation', async function () {
          ...
          const signature = await this.delegator.signTypedData(this.domain, this.types, {
          ...
          expect(await this.votes.delegates(this.delegator.address)).to.be.equal(ethers.ZeroAddress);

          const delegate = await this.votes.delegateBySig(this.delegatee, nonce, ethers.MaxUint256, v, r, s);
          const timepoint = await clockFromReceipt[mode](delegate);
          ...
          expect(await this.votes.delegates(this.delegator.address)).to.be.equal(this.delegatee.address);
          expect(await this.votes.getVotes(this.delegator.address)).to.equal(0);
          expect(await this.votes.getVotes(this.delegatee)).to.equal(weight);
          expect(await this.votes.getPastVotes(this.delegatee, timepoint - 1n)).to.equal(0);
          await mine();
        ...
        it('rejects bad delegatee', async function () {
          ...
          const delegate = await this.votes.delegateBySig(this.other, nonce, ethers.MaxUint256, v, r, s);
          const receipt = await ethers.provider.getTransactionReceipt(delegate.hash);
          ...
          expect(args.fromDelegate).to.be.equal(ethers.ZeroAddress);
          expect(args.toDelegate).to.be.equal(this.other.address);
        });
    ...
    describe('getPastTotalSupply', function () {
      ...
      it('returns 0 if there are no checkpoints', async function () {
        expect(await this.votes.getPastTotalSupply(0)).to.equal(0);
      });
      ...
      it('returns the correct checkpointed total supply', async function () {
        ...
        t0.timepoint = await clockFromReceipt[mode](t0);
        t1.timepoint = await clockFromReceipt[mode](t1);
        t2.timepoint = await clockFromReceipt[mode](t2);
        t3.timepoint = await clockFromReceipt[mode](t3);
        t4.timepoint = await clockFromReceipt[mode](t4);
        t5.timepoint = await clockFromReceipt[mode](t5);
    ...
    describe('Compound test suite', function () {
      ...
      describe('getPastVotes', function () {
        ...
        it('returns 0 if there are no checkpoints', async function () {
          expect(await this.votes.getPastVotes(this.bob, 0)).to.equal(0);
        });
