  ...
  for (const { Token, mode } of TOKENS) {
    ...
    describe(`using ${Token}`, function () {
      ...
      it('deployment check', async function () {
        ...
        // Updated: Compare token addresses instead of contract objects
      ...
      // Additional edge case: Voting from a non-delegated address should fail.
      it('should not allow voting from a non-delegated address', async function () {
        const signers = await ethers.getSigners();
        // Assuming the 6th signer has not received any NFT delegation.
        const nonDelegated = signers[5];
        await this.helper.propose();
        await this.helper.waitForSnapshot();
        await expect(this.helper.connect(nonDelegated).vote({ support: VoteType.For }))
          .to.be.reverted;
      });
    });
  }
