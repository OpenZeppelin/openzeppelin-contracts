...
async function fixture() {
  ...
  const validator = await ethers.deployContract('$RSAValidator');
  ...
  await erc7579Account.$_installModule(
    ...
    validator.target,
    ethers.AbiCoder.defaultAbiCoder().encode(['bytes', 'bytes'], [signer.publicKey.e, signer.publicKey.n]),
  );

  return { ...helper, target, signer, validator, erc7579Account, erc7579AccountAsSigner, domain };
}

describe('RSAValidator', function () {
