...
contract SampleAccount is IAccount, Ownable {
    ...
    IEntryPoint private constant ENTRYPOINT_V07 = IEntryPoint(payable(0x0000000071727De22E5E9d8BAf0edAc6f37da032));
    ...
    function validateUserOp(
        ...
        require(msg.sender == address(ENTRYPOINT_V07), "only from EP");
        // Check signature
    ...
    function execute(Mode mode, bytes calldata executionCalldata) external payable {
        require(msg.sender == address(this) || msg.sender == address(ENTRYPOINT_V07), "not auth");
...
contract ERC7579UtilsTest is Test {
    ...
    function testExecuteBatchDecodeCorrectly() public {
        ...
        vm.recordLogs();
        ENTRYPOINT_V07.handleOps(userOps, payable(_beneficiary));
    ...
    function hashUserOperation(PackedUserOperation calldata useroperation) public view returns (bytes32) {
        return useroperation.hash(address(ENTRYPOINT_V07), block.chainid);
    }
