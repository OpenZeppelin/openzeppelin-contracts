...
contract('DutchAuction', function (accounts) {
  ...
  describe('pay the beneficiary', function () {
    it('the bidder should pay the beneficiary after a bid at currentAskingPrice has been received', async function () {
      ...
      const txHash1TransactionReceipt = web3.eth.getTransactionReceipt(txHash1.tx);
      const txHash2TransactionReceipt = web3.eth.getTransactionReceipt(txHash2.tx);

      const gasForTransaction1 = txHash1TransactionReceipt.gasUsed;
      const gasForTransaction2 = txHash2TransactionReceipt.gasUsed;
      ...
      assert.equal(beneficiaryFinalBalance.toNumber(), correctBalance.toNumber(),
        'beneficiary final balance is >= to their original balance plus the bid amount minus was was spent on gas');
    });
  });
