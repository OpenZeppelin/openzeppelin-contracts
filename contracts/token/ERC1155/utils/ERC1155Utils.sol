        ...
        if (to.code.length > 0) {
            bytes4 retval = abi.decode(
                Address.functionCall(
                    to,
                    abi.encodeCall(IERC1155Receiver.onERC1155Received, (operator, from, id, value, data))
                ),
                (bytes4)
            );
    ...
    function checkOnERC1155BatchReceived(
        ...
        if (to.code.length > 0) {
            bytes4 retval = abi.decode(
                Address.functionCall(
                    to,
                    abi.encodeCall(IERC1155Receiver.onERC1155BatchReceived, (operator, from, ids, values, data))
                ),
                (bytes4)
            );
