...
contract GovernorWithTimelockExternalMock is GovernorWithToken, GovernorWithTimelockExternal {
    ...
    internal virtual override(Governor, GovernorWithTimelockExternal)
    ...
    internal virtual override(Governor, GovernorWithTimelockExternal) returns (uint256 proposalId)
    { return super._cancel(targets, values, calldatas, salt); }

    function execute(address[] memory targets, uint256[] memory values, bytes[] memory calldatas, bytes32 salt)
    public payable virtual override(Governor, GovernorWithTimelockExternal) returns (uint256 proposalId)
    { return super.execute(targets, values, calldatas, salt); }

    function state(uint256 proposalId)
    public view virtual override(Governor, GovernorWithTimelockExternal) returns (ProposalState)
