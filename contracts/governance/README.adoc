...
The {Governor} contract provides primitive to set an on-chain voting system similar to [Compound's Governor Alpha & Bravo](https://compound.finance/docs/governance).

Similarly to our other contracts, it is customizable through inheritance and comes with extensions:

* {GovernorWithTimelockInternal}: A {Governor} extension that implement an internal timelock. This requires a successful proposal to be queued before then can be executed, and adds a delay between the queueing and the execution. The governor being the timelock itself, the external calls will be performed by the {Governor} itself.

* {GovernorWithTimelockExternal}: A {Governor} extension that performs executions through a {TimelockControler}. This requires a successful proposal to be queued before then can be executed. The {TimelockControler} will enforce a delay between the queueing and the execution. With this module, proposals are executed by the external {TimelockControler} contract, which would have to hold the assets that are being governed.

* {GovernorWithTimelockCompound}: A {Governor} extension that performs executions through a compound [`Timelock`](https://github.com/compound-finance/compound-protocol/blob/master/contracts/Timelock.sol). This requires a successful proposal to be queued before then can be executed. The `Timelock` will enforce a delay between the queueing and the execution. With this module, proposals are executed by the external `Timelock` contract, which would have to hold the assets that are being governed.

In addition to modules, the {Governor} requires a few virtual functions to be implemented to your particular specifications:

* <<Governor-votingOffset-,`votingOffset()`>>: Delay (number of blocks), between the proposal, is submitted and the snapshot block used for voting. This can be used to enforce a delay after a proposal is published for users to buy tokens, or delegate their votes.
* <<Governor-votingDuration-,`votingDuration()`>>: Delay (in seconds), between the proposal, is submitted and the vote ends.
* <<Governor-maxScore-,`maxScore()`>>: Upper bound of the scale votes. Setting this to 2 will only allow simple yes/abstention/no votes. More expressive voting can be enabled by allowing users to votes on a scale from 0 to 100.
* <<Governor-requiredScore-,`requiredScore()`>>: Minimum score a proposal needs to pass. A proposal score is the average of the voter scores, weighted by their voting power. Common behavior can be reproduced by setting the `maxScore` to 2, and the `requiredScore` to 1, but you may want to allow users to vote on a scale from 0 to 100, and require a proposal to reach a score of 66 to pass.
...
* <<Governor-getVotes-address-uint256-,`getVotes(address account, uint256 blockNumber)`>>: Number of votes (voting weight) for an `account` at a specific block. This value can come from a voting token's `getPriorVotes` (implemented by Comp tokens as well as {ERC20Votes}). This can also be customized to aggregate multiple tokens, or to extract voting weight from any reputation system.

Note: Function of the {Governor} contract does NOT include access control. If you want to restrict access (for example to require a minimum balance to submit a proposal), you should add these checks by overloading the particular functions. For security reasons, the `_cancel` method is internal, and you will have to expose it (which the right access control mechanism) yourself if this is a mechanism you need.
...
{{GovernorWithTimelockInternal}}
{{GovernorWithTimelockExternal}}
{{GovernorWithTimelockCompound}}
