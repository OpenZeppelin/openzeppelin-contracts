 ...
 * {TransientSlot}: Primitives for reading from and writing to transient storage (only value types are currently supported), either at an arbitrary slot or at a slot reserved in the persistent storage layout, which lets the compiler allocate (and namespace) it.
