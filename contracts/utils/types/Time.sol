...
library Time {
     ...
     */
    function clock() internal view returns (uint48) { return uint48(block.timestamp); }

    /**
     * @dev Check if a timepoint is set, and in the past
     */
    function isSetAndPast(uint48 timepoint) internal view returns (bool) {
    ...
    function getAt(Delay self, uint48 timepoint) internal pure returns (uint32) {
        ...
        return (effect == 0 || effect > timepoint)
            ? oldValue
            : newValue;
    }
     ...
     */
    function set(uint32 delay) internal pure returns (Delay) {
        return pack(delay, 0, 0);
    }
    ...
    function update(Delay self, uint32 newValue) internal view returns (Delay) {
        ...
        return self.updateAt(newValue, clock() + setback);
    }
