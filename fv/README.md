...
> **Note**
> An API Key is required for local testing. In CI the prover runs on any pull request whose diff reaches a spec, a harness, a contract one of them verifies, or a patch in `fv/diff` applied to one, with one job per affected configuration. That selection is computed by `scripts/list-dependencies.js`.
