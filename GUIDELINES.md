...
* Custom errors should be declared following the [EIP-6093](https://eips.ethereum.org/EIPS/eip-6093) rationale whenever reasonable. Also, consider the following:
  ...
  * The domain prefix should be picked in the following order:
    1. Use `ERC<number>` if the error is a violation to an ERC specification.
    2. Use the name of the underlying component it belongs (eg. `Governor`, `ECDSA` or `Timelock`).
  ...
  * The location of custom errors should be decided in the following order:
    ...
    2. Declare the errors in the underlying interface/library if the error makes sense on its context.
    3. Declare the error in the implementation if the underlying interface/library is not suitable to do so (eg. interface/library already specified in a ERC).
    4. Declare the error in a extension if the error only happens in such extension or child contracts.
