...
A *GSN Bouncer* decides which transaction get approved and which transaction get rejected. Bouncers are a key concept within GSN. Dapps need Bouncers to prevent malicious users to spend the transactions subsidy.
...
* It needs to do accept/pre/post
...
The signature used to create the transaction must be added to the contract as a trusted signer. If it is no the same, this bouncer will not accept the transaction.
