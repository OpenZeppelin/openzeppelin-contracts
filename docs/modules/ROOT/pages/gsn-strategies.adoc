...
TIP: We wrote in-depth guide on how to setup a signing server that works with `GSNRecipientSignature`, https://forum.openzeppelin.com/t/advanced-gsn-gsnrecipientsignature-sol/1414[check it out!]
...
`_preRelayedCall` and `_postRelayedCall` are not required for all GSN strategies (`GSNRecipientSignature` leaves them empty), but are useful when your strategy involves charging end users.
