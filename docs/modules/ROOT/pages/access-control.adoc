...
In order to restrict access to some functions of your contract, you should inherit from the xref:api:access.adoc#AccessManaged[`AccessManaged`] contract provided along with the manager so that the `restricted` modifier is available to protect any externally facing function. Note you will have to set the address of the Access Manager instance (xref:api:access.adoc#AccessManaged-constructor-address-[`initialAuthority`]) in the constructor so the `restricted` modifier can check permissions by calling the correct manager.
...
WARNING: Even if an `AccessManager` defines permissions for a contract function, these won't be applied if the managed contract instance is not using the xref:api:access.adoc#AccessManageddf-restricted--[`restricted`] modifier for that function, or if its manager is a different one.
...
The `AccessManager` provides a built-in interface for configuring permission settings that can be accessed by the `AccessManager`'s `ADMIN_ROLE` members.
