...
The base `AccessControl` contract provides role-based access control, but it does not support on-chain enumeration of role members. To track which accounts hold a role, you should instead rely on the {IAccessControl-RoleGranted} and {IAccessControl-RoleRevoked} events, which can be processed off-chain. If on-chain enumeration is required, use the {AccessControlEnumerable} extension.
