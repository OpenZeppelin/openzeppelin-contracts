...
Contracts already inheriting from xref:api:access.adoc#Ownable[`Ownable`] can migrate to the Access Manager by transfering ownership to the manager. After that, the ownable contract will be called thorugh the xref:api:access.adoc#AccessManager-execute-address-bytes-[`execute`] function. Even if the caller doesn't require a delay.
