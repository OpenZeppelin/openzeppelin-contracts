...
* Access to a restricted target function is restricted to one role. A target function is defined by one https://docs.soliditylang.org/en/v0.8.20/abi-spec.html#function-selector[function selector] on one contract (called target). 
...
await manager.grantRole(MINTER, ACCOUNT, EXECUTION_DELAY, { from: initialAdmin });
```
...
WARNING: Even if an `AccessManager` defines permissions for a target function, these won't be applied if the managed contract instance is not using the xref:api:access.adoc#AccessManageddf-restricted--[`restricted`] modifier for that function, or if its manager is a different one.
