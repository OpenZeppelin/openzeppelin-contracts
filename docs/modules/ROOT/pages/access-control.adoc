...
* Roles are granted to accounts (addresses) following a many-to-many approach. This means that each user can have one or multiple roles, and that multiple users can have the same role.
...
It's recommended to keep only a single admin address secured under a multisig or governance layer. To achieve this, it is possible for the initial admin to set up all the required permissions, targets and functions, assign a new admin and finally renouncing its admin role.

Finally, the manager includes a mode where administrators can completely close a target contract. When closed, all calls to a target contract will revert.

Additionally, closing and opening contracts don't alter any of their settings, neither permissions nor delays. Particularly, the roles required for calling specific functions are not modified.
