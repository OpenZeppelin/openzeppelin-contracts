...
Although xref:api:access.adoc#AccessControl[`AccessControl`] offers a more dynamic solution for adding permissions to your contracts, decentralized protocols tend to become more complex after integrating new contract instances, requiring keeping track of permissions in each one. This increases the complexity of permissions management and monitoring across the system.
...
Better role management can be achieved with an xref:api:access.adoc#AccessManager[`AccessManager`] instance. Instead of managing each contract's permission separately, AccessManager stores all the permissions in a single contract, making your protocol easier to audit and maintain.

image::access-manager.svg[Access Manager]

The AccessManager is designed around the concept of role:

* Roles are granted to accounts (addresses) following a many-to-many approach. This means that each user can have one or multiple roles and multiple users can have the same role.
* Access to a restricted function is restricted to one role. A function is defined by one https://docs.soliditylang.org/en/v0.8.20/abi-spec.html#function-selector[function selector] on one contract (called target). 

For a call to be authorized, the caller must bear the role that is assigned to the current function (contract address + function selector).

image::access-manager-functions.svg[Access Manager functions]

=== Using `AccessManager`

OpenZeppelin Contracts provides xref:api:access.adoc#AccessManager[`AccessManager`]. A contract that can be deployed and used out of the box. It sets an initial admin in the constructor who will be allowed to perform management operations.

In order to restrict access to some functions of your contract, you should inherit from the xref:api:access.adoc#AccessManaged[`AccessManaged`] contract provided along with the manager so that the `restricted` modifier is available to protect any externally facing function. Note you will have to set the address of the Access Manager instance (xref:api:access.adoc#AccessManaged-authority--[authority]) in the constructor so the `restricted` modifier can check permissions by calling the correct manager.
...
Once the managed contract has been deployed, it is now under the manager's control, which will get the minter role assigned to an address and also allow the role to call the `mint` function.
...
Even though each role has its own list of function permissions, each role member (`address`) will have an execution delay that will dictate how long the account should wait to execute a function that requires its role. Delayed operations must be xref:api:access.adoc#AccessManager-schedule-address-bytes-uint48-[`schedule`d] first on the Access Manager before they can be executed, either directly or using the Access Manager's xref:api:access.adoc#AccessManager-execute-address-bytes-[`execute`] function.

Additionally, roles can have a granting delay that prevents adding members immediately. The Access Manager admins can set this grant delay as follows:
...
It's recommended to keep only a single admin address secured under a multisig or governance layer. To achieve this, it is possible for the initial admin to set up all the required permissions, targets, and functions, assign a new admin, and finally renounce its admin role.

For improved incident response coordination, the manager includes a mode where administrators can completely close a target contract. When closed, all calls to a target contract will revert.

Closing and opening contracts don't alter any of their settings, neither permissions nor delays. Particularly, the roles required for calling specific functions are not modified.
...
IMPORTANT: Be careful with the `ADMIN_ROLE` members, since it act as the default admin and guardian for every role, even a misbehaved guardian can cancel operations at discretion, affecting the manager's operation.

=== Manager configuration

The manager contract provides a built-in interface for configuring permission settings that can be accessed by the manager's `ADMIN_ROLE` members.
...
More specifically, these delayed functions are those for configuring the settings of a specific target contract. The delay applied to these functions can be adjusted by the manager admins with xref:api:access.doc#AccessManager-setTargetAdminDelay-address-uint32-[`setTargetAdminDelay`].
