...
```javascript
const manager = await accessManager.getAddress();

await ownable.transferOwnership(manager, { from: owner });
```
...
```javascript
// Revoke old roles
await accessControl.revokeRoke(MINTER_ROLE, account, { from: admin });

// Grant the admin role to the access manager
const manager = await accessManager.getAddress();
await accessControl.grantRole(DEFAULT_ADMIN_ROLE, manager, { from: admin });

await accessControl.renounceRole(DEFAULT_ADMIN_ROLE, admin, { from: admin });
```
