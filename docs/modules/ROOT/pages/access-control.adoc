...
The base `AccessControl` contract provides role-based access control, but it does not support on-chain enumeration of role members. To track which accounts hold a role, you should instead rely on the xref:api:access.adoc#AccessControl-RoleGranted[RoleGranted] and xref:api:access.adoc#AccessControl-RoleRevoked[RoleRevoked] events, which can be processed off-chain. If on-chain enumeration is required, use the xref:api:access.adoc#AccessControlEnumerable#AccessControlEnumerable[AccessControlEnumerable] extension.
