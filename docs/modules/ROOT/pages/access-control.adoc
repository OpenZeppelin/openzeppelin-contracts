...
For improved incident response coordination, the manager includes a mode where administrators can completely close a target contract. When closed, all calls to restricted functions in a target contract will revert.
...
WARNING: Even if a manager defines permissions for a contract function, these won't be applied if the managed contract instance is not using the xref:api:access.adoc#AccessManageddf-restricted--[`restricted`] modifier for that function, or if its manager is a different one.
...
IMPORTANT: Be careful with the members of `ADMIN_ROLE`, since it acts as the default admin and guardian for every role. A misbehaved guardian can cancel operations at will, affecting the manager's operation.
