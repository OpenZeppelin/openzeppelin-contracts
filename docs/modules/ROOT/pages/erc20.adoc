...
* If your contract does not need to hold any tokens, consider adding to the `_transfer` function a check that prevents token from being sent to the contract address. It is extremely common for users to send tokens to the contract by mistake, and then they cannot be retrieved (see, for example, https://optimistic.etherscan.io/address/0x4200000000000000000000000000000000000042#tokentxns).
