...
In some cases, like custodial or real-world asset (RWA) systems, token supply is managed off-chain. Tokens are minted only after verifying the underlying asset.

For an example of this pattern, see
xref:erc20-vault-backed-rwa.adoc[Vault-Backed ERC20 Tokens].
