...
Here’s what a preset ERC721 contract would look like, https://github.com/theadultnoble/openzeppelin-contracts/blob/master/contracts/token/ERC721/presets/ERC721PresetMinterPauserAutoId.sol[ERC721PresetMinterPauserAutoId]. Its presets include token minting with token ID and URI auto-generation, stopping all token transfers (pause) and allowing holders to burn (destroy) their tokens. The contract uses xref:access-control.adoc[Access Control] to assign access to the minting and pausing functionality. The account that deploys the contract will be granted the minter and pauser roles, as well as the default admin role.
