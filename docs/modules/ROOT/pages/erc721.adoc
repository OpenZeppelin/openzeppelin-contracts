...
Here’s what a preset ERC721 contract would look like, https://github.com/OpenZeppelin/openzeppelin-contracts/blob/master/contracts/token/ERC721/presets/ERC721PresetMinterPauserAutoId.sol[ERC721PresetMinterPauserAutoId]. Its presets include token minting with token ID and URI auto-generation, stopping all token transfers (pause) and allowing holders to burn (destroy) their tokens. The contract uses https://docs.openzeppelin.com/contracts/4.x/access-control[Access Control] to control access to the minting and pausing functionality. The account that deploys the contract will be granted the minter and pauser roles, as well as the default admin role.
