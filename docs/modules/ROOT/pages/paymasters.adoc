...
TIP: Learn more about xref:accounts.adoc#selecting-a-signer[signers] to explore different approaches to user operation sponsorship via signatures.
...
NOTE: xref:api:account.adoc#PaymasterERC20-_minTokensPerNative--[`_minTokensPerNative`] defaults to `0` in `PaymasterERC20` and should be overridden. It is a lower bound on `tokenPerNative` (same units: token-units per native coin, scaled by xref:api:account.adoc#PaymasterERC20-_tokenPerNativeDenominator--[`_tokenPerNativeDenominator`]); any operation whose `_fetchDetails` returns a `tokenPerNative` strictly below this floor is rejected with `SIG_VALIDATION_FAILED` before pre-charging the user. The example above returns `1e8`, which refuses to sponsor when 1 ETH is worth less than 100 USDC — a conservative sanity check against misconfigured oracles or extreme market conditions. Returning `0` disables the check and re-opens the "free sponsorship" footgun if `_fetchDetails` ever returns a near-zero price.
...
2. `tokenPerNative` is the number of USDC units (smallest denomination) charged per wei of gas. Combining the two feeds: `USDC_per_wei = (ETH_USD / USDC_USD) * 10^USDC_decimals / 10^18`. Multiply by `_tokenPerNativeDenominator()` to get the fixed-point value expected by xref:api:account.adoc#PaymasterERC20-_erc20Cost-uint256-uint256-uint256-[`_erc20Cost`].

NOTE: The price of the ERC-20 must be scaled by xref:api:account.adoc#PaymasterERC20-_tokenPerNativeDenominator--[`_tokenPerNativeDenominator`].
    ...
    returns (uint256 validationData, uint256 tokenPerNative)
{
    ...
    // USDC_units / wei = (ETH/USD) / (USDC/USD) * 10^USDC_decimals / 10^18, multiplied by
    // `_tokenPerNativeDenominator()` to give the fixed-point value expected by `_erc20Cost`.
    int256 scale = _tokenPerNativeDenominator().toInt256();
    int256 numerator = ETHUSD *
        (10 ** USDC_USD_ORACLE.decimals()).toInt256() *
        (10 ** USDC_DECIMALS).toInt256() *
        scale;
    int256 denominator = USDCUSD * (10 ** ETH_USD_ORACLE.decimals()).toInt256() * int256(1e18);
    int256 scaledTokenPrice = numerator / denominator;

    return (
        ETHUSDValidationData.combineValidationData(USDCUSDValidationData),
        uint256(scaledTokenPrice) // Safe upcast
    );
}
