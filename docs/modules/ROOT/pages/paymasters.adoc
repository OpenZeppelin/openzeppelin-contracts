...
NOTE: xref:api:account.adoc#PaymasterERC20-_minTokenPrice--[`_minTokenPrice`] is *abstract* in `PaymasterERC20` and must be implemented. It is a lower bound on `tokenPrice` (same units: token-units per wei, scaled by xref:api:account.adoc#PaymasterERC20-_tokenPriceDenominator--[`_tokenPriceDenominator`]); any operation whose `_fetchDetails` returns a `tokenPrice` strictly below this floor is rejected with `SIG_VALIDATION_FAILED` before pre-charging the user. The example above returns `1e8`, which refuses to sponsor when 1 ETH is worth less than 100 USDC — a conservative sanity check against misconfigured oracles or extreme market conditions. Returning `0` disables the check and re-opens the "free sponsorship" footgun if `_fetchDetails` ever returns a near-zero price.
...
2. `tokenPrice` is the number of USDC units (smallest denomination) charged per wei of gas. Combining the two feeds: `USDC_per_wei = (ETH_USD / USDC_USD) * 10^USDC_decimals / 10^18`. Multiply by `_tokenPriceDenominator()` to get the fixed-point value expected by xref:api:account.adoc#PaymasterERC20-_erc20Cost-uint256-uint256-uint256-[`_erc20Cost`].
    ...
    returns (uint256 validationData, uint256 tokenPrice)
{
    ...
    // `_tokenPriceDenominator()` to give the fixed-point value expected by `_erc20Cost`.
