...
NOTE: On mainnet `tokenPerNative` is expressed per ETH and not per wei. On most EVM chains, this native token includes 10¹⁸ units (wei). This can be customized by overriding xref:api:account.adoc#PaymasterERC20-_tokenPerNativeDenominator--[`_tokenPerNativeDenominator`].
...
contract PaymasterUSDCGuaranteed is EIP712, PaymasterERC20Guarantor, Ownable {
    ...
    bytes32 private constant GUARANTEED_USER_OPERATION_TYPEHASH =
        keccak256(
            "GuaranteedUserOperation(address sender,uint256 nonce,bytes initCode,bytes callData,bytes32 accountGasLimits,uint256 preVerificationGas,bytes32 gasFees,uint256 paymasterVerificationGasLimit,uint256 paymasterPostOpGasLimit,bytes paymasterData)"
        );
    ...
    function _getGuaranteedOperationStructHash(
        ...
        return keccak256(
            abi.encode(
                ...
                userOp.gasFees,
                // Guarantor explicitly signs the paymaster gas limits to prevent sender-controlled under-provisioning.
