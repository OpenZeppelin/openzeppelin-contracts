...
If you need support for more powerful structures than Solidity's native arrays and mappings, take a look at:

- xref:api:utils.adoc#BitMaps[`BitMaps`],
- xref:api:utils.adoc#Checkpoints[`Checkpoints`],
- xref:api:utils.adoc#DoubleEndedQueue[`DoubleEndedQueue`],
- xref:api:utils.adoc#EnumerableSet[`EnumerableSet`],
- xref:api:utils.adoc#EnumerableMap[`EnumerableMap`].
