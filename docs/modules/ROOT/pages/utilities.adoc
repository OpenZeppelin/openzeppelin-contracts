...
The xref:api:utils.adoc#StorageSlotTransient[`StorageSlotTransient`] library supports transient storage through user defined value types (https://docs.soliditylang.org/en/latest/types.html#user-defined-value-types[UDVTs]), which enables the same value types as in Solidity.
