...
IMPORTANT: The P256 library only allows for `s` values in the lower order of the curve (i.e. `s < N`) to prevent malleability. In case your tooling produces signatures in both sides of the curve, consider flipping the `s` value to keep compatibility.
