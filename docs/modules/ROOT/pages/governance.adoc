...
For all of these options, the Governor will be compatible with Tally: users will be able to create proposals, see voting periods and delays following xref:api:interfaces.adoc#IERC6372[IERC6372], visualize voting power and advocates, navigate proposals, and cast votes. For proposal creation in particular, projects can also use https://defender.openzeppelin.com/v2/#/actions/executable[Defender Transaction Proposals] as an alternative interface.
...
Generally a proposal will be created with the help of an interface such as Tally or https://defender.openzeppelin.com/v2/#/actions/executable[Defender]. Here we will show how to create the proposal using Ethers.js.
