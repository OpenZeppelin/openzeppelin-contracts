...
contract MyToken is ERC20, ERC20Permit, ERC20Votes {
    ...
    // OPTIONAL: overrides IERC6372 functions to make the token & governor timestamp-based
...
NOTE: The internal clock used by the token to store voting balances will dictate the operating mode of the governor contract attached to it. By default, block numbers are used. Since v4.9, developpers can override the xref:api:interfaces.adoc#IERC6372[ERC6372] clock to use timestamps instead of block numbers.
...
These parameters are specified in the unit defined in the token's clock. The default is number of blocks, but in the example above we overrode it to timestamp. We set votingDelay = 1 day = 86400 seconds, and votingPeriod = 1 week = 604800 seconds. With block numbers, and assuming block time of around 12 seconds, we would have set votingDelay = 1 day = 7200 blocks, and votingPeriod = 1 week = 50400 blocks.
...
contract MyGovernor is Governor, GovernorCompatibilityBravo, GovernorVotes, GovernorVotesQuorumFraction, GovernorTimelockControl {
    ...
    function votingDelay() public pure override returns (uint256) {
        return 86400; // 1 day in seconds (with a token operationg using timestamps)
    }

    function votingPeriod() public pure override returns (uint256) {
        return 604800; // 1 week in seconds (with a token operationg using timestamps)
    }
