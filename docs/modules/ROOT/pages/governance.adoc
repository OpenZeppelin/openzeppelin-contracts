...
Note that the compatibility provided doesn't override the OpenZeppelin Governor implementation, that comes with differences in the way `proposalId`s are calculated. While the original Bravo implementation stores sequential `proposalId`s, OpenZeppelin Governor stores the hash of the proposal with the purpose of keeping its data off-chain.
