...
An OpenZeppelin Governor contract is by default not interface-compatible with GovernorAlpha or Bravo, since some of the functions are different or missing, although it shares all of the same events. However, it’s possible to opt in for compatibility by inheriting from the GovernorCompatibilityBravo module, which covers the proposal lifecycle functions such as `propose` or `execute`, the voting ABI, and other getters.

Full interface-compatibility doesn't include domain-specific methods not required for composability with other OpenZeppelin contracts.

Note that the compatibility provided doesn't override the OpenZeppelin Governor implementation, that comes with differences in the way `proposalId`s are calculated. Governor stores the hash of the proposal with the purpose of keeping its data off-chain by event indexing, while the original Bravo implementation uses sequential `proposalId`s.
