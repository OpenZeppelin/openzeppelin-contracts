...
* A successful xref:api:token/ERC20.adoc#IERC20-transfer-address-uint256-[`transfer`] or xref:api:token/ERC20.adoc#IERC20-transferFrom-address-address-uint256-[`transferFrom`] of `amount` reduces the sender's xref:api:token/ERC20.adoc#IERC20-balanceOf-address-[`balanceOf`] and increases the recipient's xref:api:token/ERC20.adoc#IERC20-balanceOf-address-[`balanceOf`] by exactly `amount`.
* An account's xref:api:token/ERC20.adoc#IERC20-balanceOf-address-[`balanceOf`] only changes through explicit transfers, mints, or burns.
...
TIP: When a contract custodies tokens and may accumulate excess balance (positive rebasing, accidental transfers, airdrops), consider exposing an access-controlled recovery function similar to xref:api:token/ERC20.adoc#ERC20Wrapper-_recover-address-[`ERC20Wrapper._recover`] to retrieve the surplus. This only helps when the contract holds more than it owes, not the undercollateralization case above.
