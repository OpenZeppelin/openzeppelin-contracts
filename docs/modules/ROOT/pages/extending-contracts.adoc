...
The `super` keyword will let you call functions defined in a parent contract, even if they overridden. This mechanism can be used to add additional checks to a function, emit events, or otherwise add functionality as you see fit.

TIP: For more information on how overrides work, head to the https://solidity.readthedocs.io/en/latest/contracts.html#index-17[official Solidity documentation].
...
2. **Always** call the parent's hook in your override. This will make sure all hooks in the inheritance tree are called: contracts like xref:api:token/ERC20.adoc#ERC20Pausable[`ERC20Pausable`] rely on this behavior.
