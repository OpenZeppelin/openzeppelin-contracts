...
const domain = {
  ...
  verifyingContract: '0x<ENTRYPOINT_ADDRESS>',
};
...
NOTE: Using `getUserOpHash` directly may provide a poorer user experience as users see an opaque hash rather than structured transaction data.
